...
function DataSiswa() {
  ...
  const handleConfirmAction = () => {
    if (alertState.action === 'delete_siswa') {
  ...
  const handleAddOrUpdate = (formData) => {
    if (editData) {
      ...
      setStudents(updated);
      setEditData(null);
      showAlert('success', 'Berhasil', "Data siswa berhasil diperbarui!");
    } else {
      const newStudent = {
        id: Date.now(),
        nama: formData.namaSiswa,
  ...
  const handleDeleteStudent = (id) => {
    showAlert('confirm', 'Konfirmasi Hapus', 'Apakah Anda yakin ingin menghapus data siswa ini?', 'delete_siswa', id);
  };
  ...
  const handleImportFromExcel = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        showAlert('success', 'Berhasil', `Berhasil mengimpor ${importedStudents.length} data siswa!`);
      } catch (error) {
        showAlert('error', 'Gagal', 'Gagal membaca file Excel. Pastikan format file benar.');
        console.error(error);
      }
    };
