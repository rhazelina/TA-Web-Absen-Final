...
function DataGuru() {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      {
        ...
        'Mata Pelajaran': 'Contoh Mata Pelajaran',
        'Jabatan': 'Guru'
      }
    ];
    ...
    const fileName = 'template-data-guru.xlsx';
    XLSX.writeFile(workbook, fileName);
    ...
    reader.onload = (e) => {
      try {
        ...
        const importedData = jsonData.map((row, index) => {
          ...
          const jabatan = row['Jabatan'] || row['jabatan'] || row['Role'] || row['role'] || '';

          if (!kodeGuru || !namaGuru || !mataPelajaran || !jabatan) {
        ...
        alert(`Berhasil mengimpor ${importedData.length} data guru dari Excel!`);
      } catch (error) {
